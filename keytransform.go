...
type KeyMapping func(ds.Key) (ds.Key, error)

// KeyTransform is an object with a pair of functions for (invertibly)
// transforming keys
type KeyTransform interface {
	ConvertKey(ds.Key) (ds.Key, error)
	InvertKey(ds.Key) (ds.Key, error)
}
...
// operations.
func KeyTransformWrap(child ds.Datastore, t KeyTransform) *Datastore {
	if t == nil {
...
func (d *Datastore) Put(key ds.Key, value []byte) (err error) {
	k, err := d.ConvertKey(key)
	if err != nil {
		return errors.Wrap(err, "convert key")
	}
	return d.child.Put(k, value)
}

// Sync implements Datastore.Sync
func (d *Datastore) Sync(prefix ds.Key) error {
	p, err := d.ConvertKey(prefix)
	if err != nil {
		return errors.Wrap(err, "convert key")
	}
	return d.child.Sync(p)
}
...
func (d *Datastore) Has(key ds.Key) (exists bool, err error) {
	k, err := d.ConvertKey(key)
	if err != nil {
		return false, errors.Wrap(err, "convert key")
	}
...
func (d *Datastore) GetSize(key ds.Key) (size int, err error) {
	k, err := d.ConvertKey(key)
	if err != nil {
		return 0, errors.Wrap(err, "convert key")
	}
...
func (d *Datastore) Query(q dsq.Query) (dsq.Results, error) {
	...
	qr := dsq.ResultsFromIterator(q, dsq.Iterator{
		Next: func() (dsq.Result, bool) {
			...
			if r.Error == nil {
				ik, err := d.InvertKey(ds.RawKey(r.Entry.Key))
				if err != nil {
					r.Error = err
					return r, true
				}
				r.Entry.Key = ik.String()
			}
			return r, true
		},
		Close: func() error {
...
func (d *Datastore) prepareQuery(q dsq.Query) (naive dsq.Query, child dsq.Query, err error) {
	...
	p, err := d.ConvertKey(ds.NewKey(child.Prefix))
	if err != nil {
		return naive, child, errors.Wrap(err, "convert key")
	}
	child.Prefix = p.String()
...
orders:
	...
	for i, f := range child.Filters {
		...
		case dsq.FilterKeyCompare:
			k, err := d.ConvertKey(ds.NewKey(f.Key))
			if err != nil {
				return naive, child, err
			}
		...
		case dsq.FilterKeyPrefix:
			p, err := d.ConvertKey(ds.NewKey(f.Prefix))
			if err != nil {
				return naive, child, err
			}
